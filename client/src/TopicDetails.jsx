...
const TopicDetails = () => {
  ...
  useEffect(() => {
    const fetchTopic = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_FORUM_BACKEND_SERVER}/topics/${id}`);
        if (response.ok) {
          const data = await response.json();
          setTopic(data);
          setEditTopicText(data.topic.description);
          await fetch(`${import.meta.env.VITE_FORUM_BACKEND_SERVER}/topics/${id}`, {
            method: 'PUT',
            headers: {
    ...
    const fetchReplies = async (page) => {
      try {
        const response = await fetch(`${import.meta.env.VITE_FORUM_BACKEND_SERVER}/replies/${id}?page=${page}&pageSize=${pageSize}`);
        if (response.ok) {
          const data = await response.json();
          setReplies(data);
        } else {
    ...
    const getUser = async () => {
      ...
      if (userCookie) {
        ...
        try {
          const response = await fetch(`${import.meta.env.VITE_FORUM_BACKEND_SERVER}/users/${username}`);
          if (response.ok) {
  ...
  const handleDeleteReply = async (replyID) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_FORUM_BACKEND_SERVER}/replies/${replyID}`, {
        method: 'DELETE'
      });
      if (response.ok) {
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`${import.meta.env.VITE_FORUM_BACKEND_SERVER}/replies/${editingReply}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
